...
const MarkdownPreviewer: React.FC = () => {
  ...
  const components: any = {
    ...
    h1: ({ children }: { children: React.ReactNode }) => {
      ...
      return (
        <h1 id={`heading-${titles.indexOf(headingText)}`} className={`mt-2 mb-2 ${isDarkTheme ? 'text-white' : ''}`}>
          {children}
        </h1>
      );
    },
    h2: ({ children }: { children: React.ReactNode }) => {
      ...
      const headingId = headingText.toLowerCase().replace(/\s+/g, '-');
      return (
        <h2 id={`heading-${titles.indexOf(headingText)}`} className={`mt-5 ${isDarkTheme ? 'text-white' : ''}`}>
          {children}
        </h2>
      );
    },
    h3: ({ children }: { children: React.ReactNode }) => {
      ...
      const headingId = headingText.toLowerCase().replace(/\s+/g, '-');
      return (
        <h3 id={`heading-${titles.indexOf(headingText)}`} className={`mt-5 ${isDarkTheme ? 'text-white' : ''}`}>
          {children}
        </h3>
      );
    },
    strong: ({ children }: { children: React.ReactNode }) => (
      <div className={`mt-5 ${isDarkTheme ? 'text-white' : ''}`}>
  ...
  return (
    <div className='ml-10 mr-10 mb-10'>
      <button
        onClick={toggleTheme}
        className={`mt-5 mb-5 bg-${isDarkTheme ? 'gray-300 text-black bg-gray-200' : 'gray-700 text-white bg-gray-800'} p-2 rounded `}
      >
        Toggle Theme
      </button>
      <div className='flex flex-row'>
        <div className='basis-10/12' style={{ height: '85vh', overflow: 'scroll' }} ref={contentRef}>
